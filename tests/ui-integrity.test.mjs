...
test('active UI contains no frozen unsupported validation language', () => {
  for (const phrase of ['99.8%','94.2%','5σ','Granger Causality Test','DeepXDE predictive mode active','High-Fidelity Training']) {
    assert.equal(activeText.includes(phrase), false, `unsupported active phrase: ${phrase}`);
  }
});

test('protected synthetic visual driver remains present but must be disclosed as illustrative and dimensionless', () => {
  const viz = read('components/SolarSystemViz.tsx');
  assert.match(viz, /calculateTorqueIndex/);
